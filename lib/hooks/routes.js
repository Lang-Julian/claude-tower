...
export function mountHooks(app, { db, notifier }) {
  app.route("POST", "/api/hook", async ({ req, res }) => {
    ...
    try {
      upsertSession(db, {
        id: sessionId,
        projectKey: cwd || "unknown",
        cwd: cwd || "",
        lastSeen: ts,
        firstSeen: ts,
        status: null,
      });
    } catch { /* race with scanner is fine */ }
    ...
    if (event === "PreToolUse" && needsApproval(toolName, toolInput)) {
      ...
      try { await notifier?.onApproval?.(approval); } catch { /* notifier is best-effort */ }
  ...
  app.route("GET", "/api/hook/await/:approvalId", async ({ res, params }) => {
