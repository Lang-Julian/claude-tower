...
// Events Claude Code actually emits. Anything outside this set is rejected
// with 400 — a junk event implies the hook handler is misconfigured (or
// someone is probing the endpoint), and silently storing garbage corrupts
// the audit log.
...
export function mountHooks(app, { db, notifier }) {
  app.route("POST", "/api/hook", async ({ req, res }) => {
    ...
    if (!KNOWN_EVENTS.has(event)) {
      return sendJson(res, 400, { ok: false, error: `unknown event: ${event}` });
    }
