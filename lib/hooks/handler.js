...
async function awaitDecision(port, approvalId) {
  ...
  // Bound the total wait to prevent a buggy server from hanging the hook
  // (and therefore Claude Code itself) forever.
  const MAX_TOTAL_MS = 30 * 60 * 1000; // 30 minutes hard cap
  ...
  return { decision: "allow", reason: "decision wait exceeded 30min cap" };
}
