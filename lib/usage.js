...
// burnRate: cost per hour over the last 60 minutes (current spend velocity).
function burnRate(db) {
  const since = Date.now() - 60 * 60 * 1000;
  const row = db.prepare(`SELECT COALESCE(SUM(cost_usd), 0) AS cost FROM usage WHERE ts >= ?`).get(since);
...
export function mountUsage(app, { db }) {
  app.route("GET", "/api/usage", ({ res }) => {
    ...
    sendJson(res, 200, {
      ...
      currentBlock,
      burnRateUsdPerHour: burnRate(db),
