...
// UUID v1–v5 shape, matches randomUUID() output. Used to reject path-injection
// attempts since the approval id is also used as a queue filename in some flows.
const UUID_RE = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;
export function isValidApprovalId(id) {
...
export function mountApprovals(app, { db, notifier }) {
  ...
  app.route("GET", "/api/approvals", ({ res, url }) => {
    ...
    else rows = pendingApprovals(db, null, limit);
  ...
  // POST /api/approvals/:id/telegram-callback — internal-only route reserved
  // for a future Telegram webhook (vs the long-poll). Today we accept it for
  // symmetry, but only from local IPs since there's no per-route auth.
  app.route("POST", "/api/approvals/:id/telegram-callback", async ({ req, res, params }) => {
    const remote = req.socket?.remoteAddress;
    const isLocal = remote === "127.0.0.1" || remote === "::1" || remote === "::ffff:127.0.0.1";
    if (!isLocal) return sendJson(res, 403, { ok: false, error: "forbidden" });
    let body;
    try { body = await readJson(req); }
    catch { return sendJson(res, 400, { ok: false, error: "bad_json" }); }
    const decision = String(body.decision || "").toLowerCase();
