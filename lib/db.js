...
export function pendingApprovals(db, sessionId = null) {
  if (sessionId) {
    return db.prepare(
      "SELECT * FROM approvals WHERE decision IS NULL AND session_id = ? ORDER BY created_at"
    ).all(sessionId);
  }
  return db.prepare("SELECT * FROM approvals WHERE decision IS NULL ORDER BY created_at").all();
}
