...
// projectKey looks like "-Users-alice-my-project" → "/Users/alice/my-project"
...
export async function readSession(filePath, { processIndex } = {}) {
  // Single stat + read pair, captured atomically inside readTail.
  const { text, start, size, mtimeMs } = await readTail(filePath);
  const stat = { size, mtimeMs };
