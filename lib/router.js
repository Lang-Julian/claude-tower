...
export function createApp({ publicDir }) {
  ...
  const app = {
    ...
    broadcast(payload) {
      ...
      // Iterate over a snapshot so we can mutate the set when a client errors.
      for (const res of [...sseClients]) {
        try {
          // Drop clients whose socket has been destroyed without firing 'close'.
...
export async function readBody(req) {
  return new Promise((resolve, reject) => {
    ...
    req.on("end", () => { if (!overflowed) resolve(data); });
